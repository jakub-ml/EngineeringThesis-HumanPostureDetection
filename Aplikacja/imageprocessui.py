...
state = "stop"
# keypoints3D_l =[np.ones(99) for _ in range(40)]
...
def start_stop():
    global state
    global keypoints3D_l 
    keypoints3D_l=[np.ones(99) for elem in range(40)]
    ...
    with mp_pose.Pose(
        ...
        while cap.isOpened():
            ...
            if 0 not in landmarks_on_screen:
                ...
                X_test = scaler.transform(np.array(keypoints3D_l))
                print("X_test", X_test, X_test.shape)
                predictions = model.predict(np.array([X_test]))
                print("predictions",predictions)
                predicted_classes = np.argmax(predictions, axis=1)
                print(["inne", "odkładanie kartonu", "pakowanie", "rozpakowywanie", "sięganie po karton"][predicted_classes[0]])
...
class ImageProcessUI(object):
    ...
    def create_application_ui(self):
        state = gr.State("Stop")
        # keypoints3D_l = gr.State([np.ones(99) for _ in range(40)])
        ...
        with self.ui_obj:
            ...
            with gr.Tabs():
                with gr.TabItem("Instrukcja"):
                    with gr.Row():
                        gr.Markdown("""# Cele aplikacji:\n\n
Aplikacja ma na celu analizowanie i klasyfikowanie wykonywanych czynności na podstawie obrazu rejestrowanego przez kamery. Umożliwia konfigurację środowiska pracy, kalibrację urządzeń oraz przekształcenie punktów dwuwymiarowych w trójwymiarowe za pomocą triangulacji, co pozwala na wizualizację szkieletu człowieka. Głównym celem jest umożliwienie użytkownikowi śledzenia i identyfikacji ruchów w czasie rzeczywistym dzięki modelowi rekurencyjnemu, który kategoryzuje czynności na podstawie przetworzonych danych.
### 1. **Instrukcja**
Zakładka „Instrukcja” zawiera szczegółowe wyjaśnienie celów oraz zasad działania aplikacji. Opisuje wszystkie elementy interfejsu użytkownika, a także krok po kroku przedstawia czynności, które należy wykonać, aby poprawnie korzystać z aplikacji. Znajdują się w niej wskazówki dotyczące przygotowania środowiska, kalibracji kamer oraz uruchomienia programu. Zakładka pełni wyłącznie funkcję informacyjną i nie umożliwia wykonywania żadnych operacji związanych z algorytmami. Dzięki jasnym i przejrzystym instrukcjom, użytkownik może z łatwością zapoznać się z działaniem aplikacji i przeprowadzić wymagane etapy konfiguracji. To narzędzie wspomaga zarówno nowych, jak i zaawansowanych użytkowników w efektywnym wykorzystaniu aplikacji.\n
### 2. **Kalibracja**
Użyj tej zakładki, aby ustawić parametry kamer, w tym ich liczbę, rodzaj urządzeń, lokalizacje szachownicy oraz neutralną poze .Zakładka „Kalibracja” w aplikacji umożliwia konfigurację kamer i składa się z trzech głównych sekcji. W pierwszej sekcji, użytkownik wybiera liczbę i rodzaj urządzeń rejestrujących, korzystając z rozwijanych list i przycisku „Określ modele kamer”. Aplikacja ogranicza maksymalną liczbę kamer do czterech, ale działa poprawnie przy użyciu co najmniej dwóch urządzeń. Po wybraniu kamer aplikacja wyświetla podsumowanie wyboru w polu tekstowym, co pomaga upewnić się, że wszystkie urządzenia zostały poprawnie skonfigurowane. Druga sekcja obejmuje proces kalibracji, gdzie użytkownik umieszcza szachownicę w centrum widoku kamer, a następnie nagrywa jej obraz za pomocą przycisku „Nagraj szachownicę”. Po nagraniu należy określić liczbę kwadratów na szachownicy oraz rozmiar każdego z nich, co pozwala aplikacji na zebranie danych kalibracyjnych po naciśnięciu przycisku „Zbieraj dane szachownicy”. Dane te są używane do obliczenia parametrów zewnętrznych kamer. W trzeciej sekcji, „Neutral pose”, użytkownik nagrywa neutralną pozycję, ustawiając się w centrum nagrywanego obszaru i zachowując stabilną postawę przez kilka sekund. Po zakończeniu nagrywania aplikacja przetwarza dane, co umożliwia prawidłowe skalowanie punktów przestrzennych w triangulacji. Po przejściu przez wszystkie kroki tworzony jest plik z informacjami o kalibracji, który wystarczy wygenerować raz, chyba że zmienione zostanie środowisko nagrywania.\n
### 3. **Uruchom**
Zakładka „Uruchom” umożliwia rozpoczęcie działania programu po przeprowadzeniu poprawnej kalibracji kamer. Naciśnięcie przycisku „Uruchom detekcję” powoduje uruchomienie algorytmu, który analizuje obraz w czasie rzeczywistym. W lewym oknie aplikacji wyświetlany jest obraz z jednej z kamer z informacją o aktualnie wykonywanej czynności. W prawym oknie pojawia się trójwymiarowy wykres punktów połączonych w sposób przypominający szkielet człowieka. W tle aplikacja wykonuje kilka kroków: obrazy ze wszystkich kamer są przesyłane do algorytmu, gdzie modele pose estimation identyfikują dwuwymiarowe punkty na ciele człowieka. Następnie te punkty są przekształcane w trójwymiarowe współrzędne za pomocą triangulacji. Nowe punkty są przekazywane do modelu rekurencyjnego, który klasyfikuje wykonywaną czynność. Wyniki w postaci kategorii czynności i trójwymiarowej reprezentacji szkieletu człowieka są przesyłane z powrotem do aplikacji i prezentowane na ekranie użytkownika. Dzięki temu użytkownik może w czasie rzeczywistym obserwować analizę ruchu oraz klasyfikację czynności.\n
Aby zacząć, wybierz odpowiednią zakładkę z menu.""")  
                ...
                with gr.TabItem("Kalibracja"):
                    ...
                    with gr.Row():
                        with gr.Row():
                            ...
                            neutralpose_button = gr.Button("Zbieraj dane pozycji")
                    ...
                    neutralpose_button.click(fn=get_netral_pose, outputs=[gr.Textbox(label=f"Parametry pozy")] )#max_val, x_min, y_min, z_min
                ...
                with gr.TabItem("Uruchom"):
                    ...
                    result = gr.Textbox(label="Wykryta kategoria:")
